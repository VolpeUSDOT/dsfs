...
downstreamcoef = 0 # difference between downstream and adjacent, absent any treament

activedeactivecoef = 4 # interaction between activation and deactivation; this is the amount that speeds bounce back after deactivation.

activeup = 5 # interaction bewteen activation and location upstream. How much higher is speed upstream during activation? Positive number: small reduction in activation effect upstream; should make it close to overall activecoef.
activedown = 1 # interaction between activation and location downstream. Positive number: smaller reduction in activation effect at downstream location.
deactiveup = 2 # interaction between deactivation and location upstream. Should make smaller bounce-back upstream
deactivedown = -1 # interaction between deactivation and location downstream
...
# Activation at same site
with(datax, tapply(during.mean-before.mean, location, mean))
# Deactivation at same site
with(datax, tapply(after.mean-during.mean, location, mean))

aggregate(datax[,c("before.mean", "during.mean", "after.mean")], list(datax$location), mean)
...
# TODO: Repeat, now with studies within publications

# Set up number of publications and number of samples per publication.

n_pub = 100
n_study = 3 # number of studies within publication
nactive = 2
ndeactive = 2
nupstream = 2
ndownstream = 2
